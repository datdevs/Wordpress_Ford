...
(function ($) {
	...
	function yith_proteo_inizialize_html_elements(){
		...
		$('[class*="animate-"]').each(function () {
			var t = $(this),
				animationClass = t.attr('class').split(' ').filter(function (value) {
					return value.match(/animate-.*/);
				}).shift().replace('animate-', ''),
				animationDelayClass = null;
				if (t.attr('class').match(/aos-delay-.*/)) {
					animationDelayClass = t.attr('class').split(' ').filter(function (value) {
						return value.match(/aos-delay-.*/);
					}).shift().replace('aos-delay-', '');
				}
			if (t.is('[data-aos]')) {
				return;
			}
			t.attr('data-aos', animationClass);
			t.attr('data-aos-delay', animationDelayClass);
		});
	...
	$( document ).on( 'yith_proteo_woocommmerce_pages_title_layout', function() {
		if (typeof yith_proteo != 'undefined' && yith_proteo.yith_proteo_page_title_layout == 'outside') {
			$( 'body.woocommerce.archive' ).find( 'header.woocommerce-products-header' ).remove();
